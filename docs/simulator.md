...
The simulator builds on macOS and on Linux. The `[simulator-base]` build flags find SDL2 through `sdl2-config`, so no host paths are hardcoded.

- On Linux, the build also links OpenSSL for the simulator MD5 helper.
