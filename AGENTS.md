...
- `scripts/check_overflow_ring_thrash.py --font-dir <family folder>` regression-checks the SD glyph-miss overflow ring against `test/epubs/test_chinese.epub`. It fails if any page turn re-fetches an already-missed codepoint. See `docs/chinese-fonts.md#overflow-ring-capacity`.

## Generated Files

- Chinese rendering fixture: `test/epubs/test_chinese.epub` and its source tree `test/epubs-src/test_chinese/`.
  Both are generated by `python3 scripts/generate_chinese_test_epub.py`, which deletes and rewrites the source tree on each run.
  Edit the generator script, not the chapter XHTML.
